...
| V73 | Jacobian sign convention frozen contract: `DirectJacobianCalculator` 用 PyEIDORS runtime 约定 `J = +∂V/∂σ` (sign=+1.0); `EidorsStyleAdjointJacobian` 用 EIDORS canonical 物理约定 `J = -∂V/∂σ` (sign=-1.0). ∀ σ ! `DirectJacobianCalculator.calculate(σ) == -EidorsStyleAdjointJacobian.calculate(σ)` (tol `1e-8`); `abs()` 相等作辅助. `linearize(σ).to_dense()` 同样 signed parity. GN runtime `rhs = -jtr` (`gauss_newton_runtime.py:952`) 与 Direct 约定配对 → 物理 δσ 与 EIDORS 同号. **不可** 跨 calculator 直接 swap 而不补偿 sign；融合 (Path C) 留待未来任务 | tests/unit/test_jacobian_direct_adjoint_parity.py; src/pyeidors/inverse/jacobian/direct_jacobian.py:367,481; src/pyeidors/inverse/jacobian/adjoint_jacobian.py:104,189; B15 |
...
| T75 | x | Path C — Jacobian 体系渐进融合 (T73 后续). 步骤先后强制：(1) 抽 shared core 模块 (e.g. `pyeidors.inverse.jacobian._core`)：几何 setup (`mesh/V/V_sigma/Q_DG/DG0/cell_areas`)、forward+adjoint 解 + `_compute_field_gradients` 复用、field-product batching (numpy + torch CPU/CUDA)、`torch_dtype` / `torch_batch_all` / `device` 策略、`linearize_lazy` 矩阵-free 路径、cache key (`backend_signature_from_forward_model` 等). (2) `DirectJacobianCalculator` 改为薄 façade 调 shared core，保留 `+∂V/∂σ` runtime sign + cache manager 集成；下游 9 caller 不变. (3) `EidorsStyleAdjointJacobian` 缩为 sign-flip adapter 调同一 shared core (`return -direct_core(σ)`)，保留 `linearize_lazy`/`torch_*` 旧接口签名向后兼容. (4) 完整回归 `test_jacobian_*` + `test_one_step_rm_parity` + `test_modular` + V21/V38/V63 全 parity gate 必须仍过 (V73 contract 不动). (5) 待 Adjoint 仅剩 sign 包装 → 改名 `EidorsJacobianAdapter` 并 deprecation-warn 旧名 1 cycle，再删 `EidorsStyleAdjointJacobian` 别名. **不**触 GN runtime / RM 公式 / cache 签名 (V73 物理 δσ pairing 保持). 风险：中；多 commit；前置依赖 T73 contract test 已落地 | V73,T73,B15 |
| T76 | . | Path C → sparse solver 群 (`src/pyeidors/inverse/solvers/sparse_*.py` 7 files: `sparse_bayesian` / `sparse_bayesian_backends` / `sparse_bayesian_engine` / `sparse_map_solver` / `sparse_optimizers` / `sparse_projection` / `sparse_runtime`). 抽 `pyeidors.inverse.solvers.sparse._core`：FISTA + IRLS shared kernel, projection + coarse hierarchy (`build_coarse_hierarchy` / `compute_projection` / `estimate_lipschitz_constant`), warm-start (`_coarse_warm_start` / `_linear_warm_start_*`), GPU context resolve. `SparseBayesianBackendMixin` 折入 base, `SparseBayesianReconstructor` 子类仅持 backend-specific update. 删 legacy `sparse_bayesian.py` 若仅 alias. 全 `tests/unit/test_sparse_*` + V49 phase-2 status 必过 | V49,V73,V75,T75 |
| T77 | . | Path C → GN engine+runtime (`gauss_newton.py` entry + `gauss_newton_engine.py` 591 行 `GaussNewtonReconstructor` + `gauss_newton_runtime.py` 3694 行 50+ helpers + `matrix_free_gn.py` + `gauss_newton_line_search.py` + `gauss_newton_weights.py` + `gauss_newton_device.py`). 抽 `pyeidors.inverse.solvers.gn._core`：PETSc matrix-free Hv/PC builder (`_PETScMatrixFreeHessianContext` + `_PETScMatrixFreePCContext` + `_build_matrix_free_*`), `_JacobianActionBundle`, residual/objective/step/rollback/iteration_log/startup_cache shared helpers. `gauss_newton_runtime.py` 拆按职责至 sub-modules (`linear_system.py` / `startup_cache.py` / `measurement_space.py` / `step_size.py` / `iteration_log.py`); `GaussNewtonReconstructor` 改 thin reconstructor wrapper 调 sub-module helpers. V10/V11/V12/V15 fast PCG / matrix-free PC contract 不退, V73 `rhs=-jtr` 与 Direct sign pairing 不动. 风险：高（3694 行 + 多 caller + 多 cache cite）；强制先 contract test gate (类比 T73→T75) | V10,V11,V12,V15,V73,V75,T75 |
| T78 | . | Path C → mesh generator 群 (`mesh_generator.py` `MeshGenerator` + `simple_mesh_generator.py` `SimpleEITMeshGenerator` + `optimized_mesh_generator.py` 851 行 `OptimizedMeshGenerator`/`OptimizedMeshConverter`/`create_eit_mesh`/`load_or_create_mesh` + `mesh3d_generator.py` 1451 行 `_GeomV2HexCylinder3DMeshGenerator`/`_GeomV2TetraCylinder3DMeshGenerator`/`_LegacyTetraCylinder3DMeshGenerator`/`Cylinder3DMeshConfig`/`ElectrodeArcConfig`/`create_cylinder_3d_eit_mesh` + `mesh_converter.py` `MeshConverter`). 抽 `pyeidors.geometry._mesh_core`：electrode arc / structured sidecar (`structured_sidecar_path_for_mesh` / `validate_structured_sidecar_payload`) / surface tag / cache name (`_build_cache_name` / `_build_cache_name_3d`) / physical group helper (`_helpers.py` 已部分抽). `MeshConverter` + `OptimizedMeshConverter` 融合, 三 cylinder generator 共 base. v1/v2 API 向后兼容, V66 mesh first-load policy 不退 | V66,V73,V75,T75 |
| T79 | . | Path C → mesh cache 群 (`process_mesh_cache.py` in-process + `dolfinx_mesh_cache.py` 448 XDMF/HDF5 + `adios4dolfinx_checkpoint.py` ADIOS2 + forward `process_setup_cache.py`). 抽 `pyeidors.geometry.cache._core`：cache-key build (V36 风格 mesh content hash + electrode/protocol/background provenance) 共抽, base `MeshCacheLayer` interface (`get(key)`/`put(key, mesh)`/`invalidate(key)`/`metadata(key)`); in-process / XDMF+HDF5 / ADIOS2 / forward-setup 子类化. V36/V62/V65/V66/V67 cache signature + HDF5 unification contract 不动 | V36,V62,V65,V66,V67,V73,V75,T75 |
| T80 | . | Path C → `pyeidors.inverse.jacobian.linearized` 内 `JacobianLinearization` (eager 持 stored `adjoint_gradients`) + `LazyAdjointJacobianLinearization` (lazy 持 `fwd_model` 即时跑 adjoint solve + grad). 两类共 `matvec` / `rmatvec` / `hessian_diag` / `normal_matvec` / `as_linear_operator` / `as_petsc_mat` / `shape` / `assert_compatible` 模式. 抽 `_LinearizationBase` 持共 method, eager/lazy 仅覆 `_resolve_adjoint_block(measurement_indices) -> ndarray` (eager 切 stored, lazy `forward_solve` + `compute_field_gradients`). V7 (`matvec == to_dense @ v` / `rmatvec == to_dense.T @ r`) / V8 (`hessian_diag` 公式) / V9 (`assert_compatible` permissive empty) / V73 sign 不动. 全 `tests/unit/test_jacobian_linearization.py` (11 cases) 必过 | V7,V8,V9,V73,V75,T75 |
| T81 | . | Path C → data sweep/audit 群 (`bucket_dense_experiments.py` 1792 + `bucket_domain_audit.py` 625 + `holdout_fit_diff.py` 1249 + `holdout_point_audit.py` 357 + `factor_sweep.py` 845 + `voltage_digit_sweep.py` 410 + `visual_audit.py` 618 + `eit_digit_metrics.py` 759 + `dynamic_sequence.py` 410 + `temporal_filtering.py` 398). 模式重: 多 `*Case` / `*Row` / `*Summary` dataclass + `run` loop + JSON/HDF5/CSV dump. 抽 `pyeidors.data._sweep_core`：(a) `SweepCase` / `SweepRow` / `SweepSummary` dataclass base + 自动 dict↔dataclass 转, (b) `run_sweep(cases, compute_row, dump_target)` pipeline, (c) HDF5 + JSON + CSV dump shared (复用 `io.hdf5_artifacts`). 子类仅覆 `compute_row(case) -> Row`. V70/V71 add_noise + bucket sweep 不退, V72 test perf budget 不退 | V70,V71,V72,V73,V75,T75 |
...
| B15 | 2026-04-27 | `DirectJacobianCalculator` (`direct_jacobian.py:367` sign=+1.0, `:481` 无 minus) 与 `AdjointJacobianCalculator` (`adjoint_jacobian.py:104` sign=-1.0, `:189` 显式 `-np.sum`) 返回 J 整体反号. `linearized.py:127,159` matvec/rmatvec 应用 `self.sign` → 反号传递. GN runtime `gauss_newton_runtime.py:952` `rhs=-jtr` 仅与 Direct 自洽；切到 Adjoint (`set_jacobian_calculator`) 后 δσ 静默反号. Production 默认 Direct 故 V21/V38 通过；无 cross-method parity 测试覆盖此切换 | V73,T73 |
