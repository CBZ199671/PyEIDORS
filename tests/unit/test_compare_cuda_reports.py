...
def test_compare_cuda_reports_handles_2d_payload(tmp_path: Path):
    ...
    cpu.write_text(json.dumps({
        "stage_timings": {"forward_homogeneous": 2.0, "forward_phantom": 2.0, "difference_reconstruct": 4.0},
        "perf_summary": {"execution_profile": "cpu"},
    }), encoding="utf-8")
    gpu.write_text(json.dumps({
        "stage_timings": {"forward_homogeneous": 1.0, "forward_phantom": 1.0, "difference_reconstruct": 3.0},
        "perf_summary": {"execution_profile": "cuda"},
    }), encoding="utf-8")

    proc = subprocess.run(
        [sys.executable, str(_script_path()), "--cpu", str(cpu), "--gpu", str(gpu), "--kind", "2d"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_compare_cuda_reports_handles_3d_payload(tmp_path: Path):
    ...
    proc = subprocess.run(
        [sys.executable, str(_script_path()), "--cpu", str(cpu), "--gpu", str(gpu), "--kind", "3d"],
        capture_output=True,
        text=True,
        check=False,
    )
