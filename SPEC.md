...
| V73 | ∀ σ, `DirectJacobianCalculator.calculate(σ)` ≡ `AdjointJacobianCalculator.calculate(σ)` (numerical, tol `1e-8`); `linearize(σ).to_dense()` 同两计算器相等. Production GN runtime `rhs = -jtr` (`gauss_newton_runtime.py:952`) 假设 J sign 唯一 → 任一 calculator 切换 ! 不得静默反号 δσ. Cross-method parity test 强制 | future tests/unit/test_jacobian_direct_adjoint_parity.py; src/pyeidors/inverse/jacobian/direct_jacobian.py:367,481; src/pyeidors/inverse/jacobian/adjoint_jacobian.py:104,189; B15 |
...
| T73 | . | 统一 Direct ↔ Adjoint Jacobian sign 约定 (建议 Adjoint 内部去掉 `_assemble_numpy:189` 显式 minus + 设 `linearize` `sign=+1.0`，保留 Direct 约定与 production runtime 自洽) + 加 cross-method parity 测试 (dense + linearization.to_dense 双比) | V73,B15 |
