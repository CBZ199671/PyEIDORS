...
def test_v677_runtime_auto_uses_measurement_mode_and_state_offset() -> None:
    metadata = {
        "dynamic_kalman_enabled": True,
    ...
    assert dynamic["effective_mode"] == "measurement"
    assert dynamic["fallback_reason"] == ""


def test_v677_runtime_auto_falls_back_to_fast_image_with_reason() -> None:
    metadata = {
        "dynamic_kalman_enabled": True,
        "dynamic_kalman_mode": "auto",
    ...
    assert dynamic["fallback_reason"] == "cached_jacobian_or_measurement_unavailable"
    assert dynamic["total_latency_frames"] == 2


def test_v677_runtime_auto_respects_measurement_state_product_budget() -> None:
    metadata = {
        "dynamic_kalman_enabled": True,
        "dynamic_kalman_mode": "auto",
