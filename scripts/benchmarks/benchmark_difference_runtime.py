...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--background", type=float, default=0.008, help="Background conductivity."
    )
    parser.add_argument(
        "--phantom-center",
        type=float,
    ...
    parser.add_argument(
        "--measure-cold", action="store_true", help="Measure cold timing (first call)."
    )
    parser.add_argument(
        "--measure-warm",
        action="store_true",
    ...
    parser.add_argument(
        "--plot-out", type=Path, default=None, help="Optional plot output path."
    )
    parser.add_argument(
        "--memory-stats",
        action="store_true",
...
def compute_difference(
    target: np.ndarray, baseline: np.ndarray, normalized: bool
) -> np.ndarray:
...
def build_measurement_weights(
    ...
    weights = np.array(reference, dtype=np.float64, copy=True)
...
def solve_single_step_delta(
    ...
    solver_space: Optional[str] = None,
    ...
    if solver_choice == "measurement":
        ...
        if system is None:
            ...
            y = _solve_with_cached_factor(
                system, lhs, rhs, payload=payload, persist=True
            )
        ...
        if system is None:
            ...
            delta = _solve_with_cached_factor(
                system, lhs, rhs, payload=payload, persist=False
            )
...
def predict_difference(
    system: EITSystem, image: EITImage, baseline_meas: np.ndarray, normalized: bool
) -> np.ndarray:
...
def optimize_step_size(
    ...
    def objective(step: float) -> float:
        test_img = build_single_step_image(
            system.fwd_model, baseline_image, delta, step, args.conductivity_bounds
        )
...
def timed_single_step(
    ...
    delta, _ = solve_single_step_delta(
        system.fwd_model, baseline_image, diff, baseline_meas, args
    )
    if args.step_size_calibration:
        step_size = optimize_step_size(
            ...
            diff,
            baseline_meas,
            args.normalized_diff,
            args,
        )
    else:
        step_size = 1.0
    _ = build_single_step_image(
        system.fwd_model, baseline_image, delta, step_size, args.conductivity_bounds
    )
...
def main() -> None:
    ...
    for refinement in args.refinements:
        ...
        if args.measure_warm:
            timed_single_step(
                system, baseline_image, baseline_meas.meas, target_meas.meas, args
            )
            times: List[float] = []
            peaks: List[float] = []
        ...
        if args.compare_solvers and refinement == compare_refinement:
            diff_target = compute_difference(
                target_meas.meas, baseline_meas.meas, args.normalized_diff
            )
            ...
            def _solve_with_delta(delta: np.ndarray) -> Tuple[float, np.ndarray]:
                ...
                pred = predict_difference(
                    system, img, baseline_meas.meas, args.normalized_diff
                )
            ...
            delta_rel = float(
                np.linalg.norm(delta_meas - delta_param)
                / (np.linalg.norm(delta_param) + 1e-12)
            )
            pred_rel = float(
                np.linalg.norm(pred_meas - pred_param)
