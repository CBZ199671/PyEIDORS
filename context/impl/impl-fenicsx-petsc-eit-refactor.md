...
### Post-v1 Follow-up (G1–G8, identified 2026-04-20 review of commit 654fe22)

本轮 baseline 与 PETSc/DOLFINx 官文对齐，但跨 GN iter 的 PC 复用未利用，matrix-free Hessian PC 缺 NOSER diag 自动导出，cache/linearization 指纹可再收紧。优先序如下，**G4/G6 经二次核验不是 bug，不要重开**。

- [x] **G1 (TOP PRIORITY): Persist KSP across GN iterations — DONE 2026-04-20.** `ForwardKSPSession` 长驻于 `EITForwardModel`；`_acquire_forward_ksp_bundle` 在结构性指纹匹配时走 `setOperators(A_new)` + `setReusePreconditioner`，否则走旧的 `_make_petsc_solver_bundle` 新建。`LinearBackendConfig` 新增 `forward_pc_refresh_policy` (`auto|never|always|lag`) + `forward_pc_refresh_iter_threshold` + `forward_pc_refresh_lag`。`backend_signature_from_forward_model` + `_base_cache_payload` 纳入三字段。诊断扩展：`forward_pc_session_reused`、`forward_pc_refresh_triggered`、`forward_pc_refresh_reason`、`forward_pc_session_solves`、`forward_pc_session_total_setups`、`forward_pc_last_iter_count`、`forward_pc_refresh_policy`。Structural fingerprint 刻意不依赖 `_petsc_backend_info` 的 lazy mat/vec type 字段，避免首次 solve 后污染。新增测试 `tests/unit/test_forward_ksp_session_reuse.py` 覆盖 reuse/never/lag/iter-threshold/结构性变更/`reuse_preconditioner=False`/`as_bundle`+`record_solve` roundtrip。Validation: 57 targeted forward/PETSc tests pass; `forward` shard PASS (16 files, 18.196s); `inverse-gn` shard PASS (32 files, 143.572s)。**G8 随 G1 自动解决**（默认 `reuse_preconditioner=True` 现在跨 GN iter 真的生效）。 `_solve_with_petsc` (`src/pyeidors/forward/eit_forward_model.py:1497`) 以 `sigma_hash` 入 `cache_manager.get_or_compute`，sigma 变即丢 KSP → GAMG coarse hierarchy 每 iter 重建。`ksp.setReusePreconditioner(True)` 在新 KSP 上设（L1366）为 no-op：PETSc 此 flag 只在同一 KSP 经 `setOperators(A_new)` 时生效。**Fix:** forward model 持有 `ForwardKSPSession`（不进 sigma-keyed cache），每 iter `ksp.setOperators(A_new)` + `setReusePreconditioner(True)` 跳 `PCSetUp`；监控 `ksp.getIterationNumber()`，超阈值则短暂 `setReuse(False)` 刷一次 PC。新增 `LinearBackendConfig.forward_pc_refresh_policy: lag_k | auto | never`。Refs: [KSPSetReusePreconditioner](https://petsc.org/release/manualpages/KSP/KSPSetReusePreconditioner/), [KSPSetOperators](https://petsc.org/release/manualpages/KSP/KSPSetOperators/).
- [x] **G5: Auto NOSER diag from JacobianLinearization — DONE 2026-04-20.** `JacobianLinearization.hessian_diag(measurement_weights, alpha, regularization_diag, floor)` 直接由 `grad_u_all` + `adjoint_gradients` + `cell_areas` 契约计算 `diag(JᵀWJ) [+ αR_diag]`，不需 materialize dense J。`_JacobianActionBundle` 增 `linearization` 字段；`_solve_linear_system_fast` 在 operator 路径下把 `W_sqrt² = measurement_weight_np²` 传给 `hessian_diag` 得 full `W = W_sqrt²` 的 diag，作为 `auto_hessian_diag_fn` 回调注入 `_operator_diag_preconditioner`。后者新增 fallback tier：当 explicit/noser/prior/R_diag 全缺而 `auto_hessian_diag_fn` 可用时，用 linearization 得到的 full diag，meta `matrix_free_pc_source="auto_linearization_diag"`；失败时落回 identity 并记录 reason。测试新增 `test_hessian_diag_matches_dense_reference`、`test_hessian_diag_applies_measurement_weights_and_regularization`、`test_hessian_diag_handles_negative_sign_and_floor`、`test_hessian_diag_rejects_shape_mismatch`、`test_fast_solver_auto_hessian_diag_from_jacobian_linearization`、`test_auto_hessian_diag_respects_measurement_weights`、`test_explicit_noser_diag_still_wins_over_auto_linearization_diag`。Validation: 59 targeted GN tests pass; `inverse-gn` shard PASS (32 files, 137.986s)。 `_operator_diag_preconditioner` (`src/pyeidors/inverse/solvers/gauss_newton_runtime.py:303`) 无 `noser_diag`/`hessian_diag` attr 即降为 identity，PCG 收敛退化。`diag(JᵀWJ)_e = Σ_m w_m·(∂V_m/∂σ_e)²` 可由已存的 `grad_u_all` + `adjoint_gradients` 直接装配，不必 materialize dense J。**Fix:** `JacobianLinearization.hessian_diag(weights=None) -> ndarray` + 在 `_operator_diag_preconditioner` 缺 explicit attr 时自动调用。零 FE solve 成本的收敛改善。
- [x] **G2 + G7: Cache / linearization fingerprint hygiene — DONE 2026-04-20.**
  - **G2:** `build_process_forward_setup_key` 去掉 `mesh_runtime_id` 参数；新增 `mesh_content_hash` fallback + missing-identifier 守卫（`mesh_file`/`mesh_content_hash` 必有其一）。EITForwardModel `__init__` 新增 `_hash_mesh_content(dolfinx_mesh)`：优先读 `mesh_file`；为空时通过 DOLFINx geometry.x + topology.connectivity(tdim, 0).array 的 SHA-256 作 in-memory 内容指纹，fake mesh 无 geometry 时返回空串不触发。
  - **G7:** `JacobianLinearization` 加 `sigma_fingerprint: str = ""` + `assert_compatible(sigma_fingerprint)` permissive guard（空串任一方时 skip）；`linearized.py` 导出 `compute_sigma_fingerprint(sigma)` 计算 conductivity SHA-256。`DirectJacobianCalculator.linearize()` 自动填入 fingerprint。
  - 测试新增 `tests/unit/test_forward_process_setup_cache.py`（8 cases: missing identifier raises、内容 hash 改变产生新 key、coords/connectivity 变化各自触发新 hash）+ `test_jacobian_linearization.py` 新 4 cases（empty 指纹 permissive、match 通过、mismatch 抛错、`compute_sigma_fingerprint` 稳定性）。Validation: 55 targeted tests PASS；`forward` shard PASS (17 files, 18.508s)；`inverse-gn` shard PASS (32 files, 140.213s)。
- [x] **G8: `reuse_preconditioner=True` 默认与现状不符（G1 落地后自动解决）— DONE via G1.** 诊断记录 `applied=True` 但跨 GN iter KSP 已丢 → 复用不发生。**Interim:** G1 前把默认改 False 或加诊断说明 `"reuse applied within single solve only; KSP not persisted across GN iterations"`；G1 后默认保持 True。
- [x] **G3: PETSc KSP for matrix-free Hessian — DONE 2026-04-20.** `gauss_newton_runtime.py` 导入 `petsc4py as _PETSc`（可选）；新增 `_solve_matrix_free_hessian_via_petsc(h_op, rhs, m_op, rtol, maxiter)` 用 `MATSHELL (createPython) + KSP(cg) + PCSHELL (pc.setPythonContext)` 求解 matrix-free Hessian，底层 context 类 `_PETScMatrixFreeHessianContext` 应用 `Hv`、`_PETScMatrixFreePCContext` 应用 `M^{-1} r`，都桥接到既有 SciPy `LinearOperator`。`_solve_pcg` 读 `reconstructor.matrix_free_ksp_backend ∈ {scipy, petsc, auto}`；`auto` 在 petsc4py 可用时选 `petsc`；失败/缺失自动 fallback 到 SciPy `cg` 并记 `matrix_free_ksp_backend_fallback_reason`。Meta 新 `matrix_free_ksp_backend_requested/effective`。默认仍为 `scipy` 保持向后兼容。测试：`test_matrix_free_ksp_backend_defaults_to_scipy_when_unset`、`test_matrix_free_ksp_backend_petsc_matches_scipy_reference`（真 petsc4py）、`test_matrix_free_ksp_backend_petsc_falls_back_when_unavailable`（`_PETSc=None` monkeypatch）、`test_solve_matrix_free_hessian_via_petsc_direct_call`（SPD 参考解）。Validation: 18 targeted fast-solver tests PASS；`inverse-gn` shard PASS (32 files, 136.418s)。 `_solve_pcg` (`src/pyeidors/inverse/solvers/gauss_newton_runtime.py:1056`) 使用 `scipy.sparse.linalg.cg`；`JacobianLinearization.as_petsc_mat()` (`src/pyeidors/inverse/jacobian/linearized.py:225`) 已暴露 PETSc MATSHELL/Python matrix 但 runtime 未接。丢 GPU/MPI CG 路径与 PETSc 收敛工具。**Fix:** inverse Hv 走 PETSc `KSP(CG) + MATSHELL(JacobianLinearization) + PCPYTHON/PCSHELL`，shell inverse/Pmat 统一契约。

**G4 / G6 — 原判 NOT bug，已做非关键改进：**
- [x] **G4 — DONE 2026-04-20 (naming + test coverage).** `prev_jacobian_np` 全文件 rename 为 `prev_jacobian`（holds `JacobianLinearization`/`LinearOperator`/`ndarray`，`_np` 后缀误导）。新增 `test_run_reconstruction_reuses_linearized_jacobian_across_iterations`：`max_iterations=4`、`jacobian_update_every=2`、`jacobian_reuse_tol=10.0`，验证 `linearize()` 仅被调用 2 次而非 4 次，4 次 fast solver 调用拿到的 `J_weighted_np` 都是同一个 `JacobianLinearization` 对象，verbose 输出至少 2 条 "reused Jacobian" 记录。
- [x] **G6 — DONE 2026-04-20 (threshold tuning).** `LinearBackendConfig.forward_mat_solve_min_patterns: int = 0` 新增可选阈值；`_should_use_mat_solve` 在 `auto` 模式多加一个 `min_patterns == 0 or n_patterns >= min_patterns` 守卫，避免小 batch 下 MATDENSE B/X 分配开销摊不平。`from_dict` / `_base_cache_payload` 同步。测试 `test_should_use_mat_solve_respects_min_patterns_threshold`、`test_should_use_mat_solve_min_patterns_ignored_in_on_mode`、`test_should_use_mat_solve_min_patterns_defaults_to_current_behaviour`。Validation: 60 targeted tests PASS; `forward` shard PASS (17 files, 18.116s); `inverse-gn` shard PASS (32 files, 133.98s)。

**优先序（用户确认）：** G1 → G5 → G2+G7 同批 → G8（随 G1 自然解决） → G3 深改末位。G6/G4 非必做。

**Durable fallback:** 完整同步也保存在 `C:\Users\huolo\.claude\projects\--wsl-localhost-Ubuntu-22-04-home-tom-workspace-PyEidors-wsl2\memory\project_fenicsx_refactor_next_plan.md`（user-scoped memory）。
