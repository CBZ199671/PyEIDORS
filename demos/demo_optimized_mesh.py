...
def demo_electrode_positions():
    ...
    for i, (name, config) in enumerate(configs):
        ...
        for j, (start, end) in enumerate(positions):
            ...
            ax.plot(
                x_elec, y_elec, "b-", linewidth=3, label="Electrode" if j == 0 else ""
            )
            mid_angle = (start + end) / 2
            ax.text(
                1.1 * np.cos(mid_angle),
                1.1 * np.sin(mid_angle),
                str(j + 1),
                ha="center",
                va="center",
                fontsize=8,
    ...
    print(
        "✅ Electrode position configuration demo complete, saved as electrode_positions_demo.png"
    )
        ...
        for i, (name, config) in enumerate(configs):
            ...
            if hasattr(mesh, "coordinates"):
                ...
                for cell in cells:
                    ...
                    ax.plot(
                        triangle[:, 0], triangle[:, 1], "b-", alpha=0.3, linewidth=0.5
                    )
...
def demo_convenience_functions():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if hasattr(mesh, "num_vertices"):
            print(
                f"   ✅ Successfully created mesh: {mesh.num_vertices()} vertices, {mesh.num_cells()} cells"
            )
        else:
        ...
        print(
            f"   📁 Generated files: {len(msh_files)} .msh, {len(xdmf_files)} .xdmf, {len(ini_files)} .ini"
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for name, config in mesh_configs:
            ...
            if hasattr(mesh, "num_vertices"):
                ...
                print(
                    f"     Vertices: {n_vertices}, Cells: {n_cells}, Density: {density:.1f} cells/unit²"
                )
        ...
        if results:
            ...
            print(
                "   ✅ Mesh quality analysis complete, saved as mesh_quality_demo.png"
            )
...
def run_demo():
    ...
    try:
        ...
        print(
            "   - electrode_positions_demo.png: Electrode position configuration comparison"
        )
