...
**3D GREIT parity note:** T40..T50 code path complete: `GREIT3D_distribution` + finite-target `vh/vi` + `desired_solution_fn`/`D` + `calc_GREIT_RM` + NF search + HDF5 `keep_model_components` + parity diagnostics + common-config warmup + T49 48e/5936 surrogate gate. Linearized GREIT-RM v0 (`Y = T @ J.T`, `D ≈ T`) remains explicit non-parity mode. No UI/docs may claim "EIDORS同款 / official-equivalent / perfect stable 3D GREIT" until external MATLAB/EIDORS fixture gate passes with `official_eidors_fixture=true` and `official_equivalence_claim_allowed=true`.
...
| V50 | 3D GREIT current status: T40..T50 done: source-map + `GREIT3D_distribution` + finite-target `vh/vi` + `desired_solution_fn`/`D` + `calc_GREIT_RM` + NF/image-SNR search + HDF5 `keep_model_components`/cache schema + parity diagnostics + common-config warmup + 48e/5936 T49 surrogate gate. Still NOT official-equivalent: no external MATLAB/EIDORS 48e fixture report in repo; T49 `official_equivalence_claim_allowed=false`. Linearized RM (`Y=T@J.T`, `D≈T`) remains explicit non-parity mode. Production "EIDORS同款" claim ! forbidden until external official fixture gate passes | src/pyeidors/inverse/greit.py; tests/unit/test_greit_rm.py; tests/unit/test_greit_eidors_final_benchmark.py; reports/runtime_benchmarks/greit_eidors_parity_48e_5936_t49_20260426/summary.json; B11 |
...
3D GREIT official-equivalence gate: all rows T40..T50 must be `x`, V55..V65 must hold, AND external MATLAB/EIDORS fixture report must set `official_eidors_fixture=true` + `official_equivalence_claim_allowed=true` before UI/docs/papers may say "EIDORS同款", "official-equivalent", "perfect stable 3D GREIT".
