...
- 11 个统计样本，经典/Robin 交替先后顺序；
- 每个样本批量执行 16 次同一操作后归一化，降低微秒级计时抖动；
- 不计时 runtime/allocator 预热；
- 冷态：每次新建本公式因子状态并首次求解全部 16 RHS；
- setup：在同一个冷态操作内部成对计量的状态构建分量，不是“热态”；
- 热态复用：每个公式独立建立一个不计时状态，仅复用自己的状态求解 16 RHS；
- 明确 `cross_formulation_cache_reuse=false`，经典结果不供 Robin 使用，反之亦然。

先看绝对冷/热关系。下表是 7 组案例时间的几何平均；仅用于同一求解器、同一公式
的冷态与热态比较，不用于 MATLAB、SciPy、NGSolve 之间的绝对性能排名。

| 求解器 | 公式 | 冷态（µs） | 热态复用（µs） | 冷态 / 热态 | 7 组最小加速 |
|---|---|---:|---:|---:|---:|
| PyEIDORS/DOLFINx | Classic | `740.63` | `29.04` | `25.51×` | `20.55×` |
| PyEIDORS/DOLFINx | Robin | `226.85` | `16.85` | `13.47×` | `8.60×` |
| NGSolve | Classic | `975.23` | `43.38` | `22.48×` | `18.56×` |
| NGSolve | Robin | `308.16` | `17.42` | `17.69×` | `7.52×` |
| EIDORS | Classic | `309.41` | `28.35` | `10.91×` | `9.08×` |
| EIDORS | Robin | `356.28` | `5.71` | `62.35×` | `47.27×` |

因此所有 42 个“案例 × 求解器 × 公式”组合都满足冷态中位数大于热态复用中位数。
此前看起来“PyEIDORS 冷态更快”，是把不同阶段的 `Robin/Classic` 比值当成绝对速度
横向比较；`0.306 < 0.580` 只表示 Robin 在冷态相对 Classic 的优势更大，不表示冷态
的绝对用时更小。

下表才是同一阶段内的 `Robin time / Classic time` 几何平均；小于 1 表示 Robin 更快。

| 求解器 | 冷态 | setup | 热态复用 | Robin 更快的案例数（冷 / setup / 复用） |
|---|---:|---:|---:|---:|
| PyEIDORS/DOLFINx | `0.306` | `0.275` | `0.580` | 7 / 7 / 7 |
| NGSolve | `0.316` | `0.244` | `0.402` | 7 / 7 / 7 |
| EIDORS | `1.152` | `1.400` | `0.202` | 0 / 0 / 7 |

因此，在这些小矩阵上：

- PyEIDORS 的 Robin 路径冷态约少 `69%` 时间、setup 约少 `72%`、热态复用
  约少 `42%`；
- NGSolve 的 Robin 在冷态、setup 和热态复用的 7/7 组中都更快；
- EIDORS 的 Robin 冷态和 setup 在 7/7 组中更慢，因为需要 `A_R` sparse LU、15 个响应基 RHS
  和一个 dense reduced LU；一旦状态建好，15 维约化回代在 7/7 案例都显著快于经典
  增广系统复用。

所有计时同时保留中位数、IQR、最小值、最大值和原始 11 个批量归一化样本；冷态还
保存配对的 setup 与 first-solve 分量。小网格微秒结果主要说明当前实现的固定开销结构，
不应外推为大型 2D/3D 稀疏系统的复杂度结论。
...
- 42 条公平计时记录完整，全部为 11 样本 × 16 归一化操作、16 RHS、交替顺序、
  成对冷态分解、无跨公式 cache；
- 42/42 记录满足 `cold_median_seconds > warm_reuse_median_seconds`；
