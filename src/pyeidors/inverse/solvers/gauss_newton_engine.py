...
from .gauss_newton_weights import (
    build_weight_reference,
    difference_with_baseline,
    scale_baseline_to_measured,
)
...
class GaussNewtonReconstructor:
    ...
    def reconstruct(self,
        ...
        with tqdm(total=self.max_iterations, disable=not self.verbose) as pbar:
            for iteration in range(self.max_iterations):
                ...
                img_current = EITImage(elem_data=sigma_array, fwd_model=self.fwd_model)
                ...
                if self.clip_values is not None:
                    function_set_array(
                        sigma_current,
                        np.clip(sigma_array, self.clip_values[0], self.clip_values[1]),
                    )
                    sigma_array = function_get_array(sigma_current)
    ...
    def _ensure_measurement_weights(self, sigma_function: fem.Function) -> None:
        ...
        reference_vector = build_weight_reference(
            ...
            measured_vector=self._measured_vector,
