...
def test_resolve_torch_dtype_and_init_cover_aliases_and_auto_device_paths(monkeypatch: pytest.MonkeyPatch):
    assert EidorsStyleAdjointJacobian._resolve_torch_dtype(None) == torch.float64
    assert EidorsStyleAdjointJacobian._resolve_torch_dtype(torch.float32) == torch.float32
    ...
    monkeypatch.setattr(EidorsStyleAdjointJacobian, "_setup", lambda self: setattr(self, "_setup_called", True))
    monkeypatch.setattr(base_module.fem, "Function", lambda _space: SimpleNamespace(x=SimpleNamespace(array=np.zeros(1, dtype=float))))
    fake_fwd_model = SimpleNamespace(V_sigma=SimpleNamespace(), pattern_manager=SimpleNamespace(n_meas_total=1))
    monkeypatch.setattr(torch.cuda, "is_available", lambda: False)
    if getattr(torch.backends, "mps", None) is not None:
        monkeypatch.setattr(torch.backends.mps, "is_available", lambda: True)
        jac_mps = EidorsStyleAdjointJacobian(fake_fwd_model, use_torch=False, device=None)
    ...
    jac_explicit = EidorsStyleAdjointJacobian(fake_fwd_model, use_torch=False, device="cpu")
...
def test_setup_and_compute_field_gradients_cover_torch_and_callable_interpolation_points(
    ...
    monkeypatch.setattr(
        adjoint_module.fem,
        "functionspace",
        lambda _mesh, _desc: SimpleNamespace(element=SimpleNamespace(interpolation_points=lambda: np.array([[0.0, 0.0]]))),
    )
    ...
    monkeypatch.setattr(adjoint_module.fem_petsc, "assemble_vector", lambda _form: _FakeCellVec([2.0, 3.0]))
    EidorsStyleAdjointJacobian._setup(jac)
    np.testing.assert_allclose(jac.cell_areas, np.array([2.0, 3.0], dtype=float))
    np.testing.assert_allclose(jac.cell_areas_t.cpu().numpy(), np.array([2.0, 3.0], dtype=np.float32))

    jac.V = SimpleNamespace(name="V")
    jac.Q_DG = SimpleNamespace(element=SimpleNamespace(interpolation_points=lambda: np.array([[0.0, 0.0]])))
    jac.gdim = 2
    monkeypatch.setattr(adjoint_module.fem, "Function", _InterpFunction)
    monkeypatch.setattr(adjoint_module.fem, "Expression", lambda grad_value, points: SimpleNamespace(grad_value=grad_value, points=points))
    monkeypatch.setattr(adjoint_module.ufl, "grad", lambda u_fun: float(np.sum(u_fun.x.array)))
...
def test_measurement_patterns_and_numpy_torch_assembly_cover_remaining_paths(monkeypatch: pytest.MonkeyPatch):
    jac = EidorsStyleAdjointJacobian.__new__(EidorsStyleAdjointJacobian)
    jac.fwd_model = SimpleNamespace(
        n_elec=2,
    ...
    assembled_np = EidorsStyleAdjointJacobian._assemble_numpy(jac, grad_u_all, grad_adj_all)
    expected = np.array(
        [
            [-5.0, -6.0],
            [-2.0, -3.0],
    ...
    assembled_torch = EidorsStyleAdjointJacobian._assemble_torch(jac, grad_u_all, grad_adj_all)
    np.testing.assert_allclose(assembled_torch, expected, atol=1e-6, rtol=1e-6)

    jac.torch_batch_all = True
    monkeypatch.setattr(jac, "_assemble_torch_all", lambda gu, ga: np.array([[42.0]], dtype=float))
    np.testing.assert_allclose(EidorsStyleAdjointJacobian._assemble_torch(jac, grad_u_all, grad_adj_all), np.array([[42.0]]))

    jac.torch_batch_all = False
    assembled_all = EidorsStyleAdjointJacobian._assemble_torch_all(jac, grad_u_all, grad_adj_all)
...
def test_calculate_dispatches_torch_path(monkeypatch: pytest.MonkeyPatch):
    ...
    jac.use_torch = True
    jac._compute_field_gradients = lambda _fields: grad_u_all if calls["count"] == 1 else grad_adj_all
    jac._measurement_to_current_patterns = lambda: np.array([[1.0], [-1.0]], dtype=float)
