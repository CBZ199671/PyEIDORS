    ...
    recon = SimpleNamespace(
        ...
        fwd_model=SimpleNamespace(
            fwd_solve=lambda _img: (
                SimpleNamespace(meas=np.array([1.0, 1.0], dtype=float)),
                None,
            ),
        ),
    )
...
def test_best_homog_bounds_and_estimate_exception(monkeypatch: pytest.MonkeyPatch):
    ...
    assert gn_runtime._best_homog_bounds(recon_scalar, 2.0) == pytest.approx(
        (0.4, 10.0)
    )

    recon_clipped = _recon(clip_values=(0.5, 1.5))
    assert gn_runtime._best_homog_bounds(
        recon_clipped, np.array([2.0, 3.0], dtype=float)
    ) == (0.5, 1.5)
...
def test_prepare_prior_and_difference_step_size_reason_paths(
    ...
    np.testing.assert_allclose(
        recon._prior_data, np.array([0.2, 0.3, 0.4], dtype=float)
    )
    ...
    _, missing_prior_info = gn_runtime._apply_difference_step_size(
        _recon(
            _measurement_space_type="difference", difference_step_size_mode="optimize"
        ),
        sigma_final=sigma_final,
        measured_vector=measured,
    )
...
def test_compute_objective_build_linear_system_and_step_selection(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(
        gn_runtime,
    ...
    np.testing.assert_allclose(
        rt_r_de.detach().cpu().numpy(), np.array([2.0, 4.0], dtype=float)
    )

    recon_linear = _recon(
        R_torch=torch.eye(2, dtype=torch.float64), use_prior_term=True
    )
    ...
    np.testing.assert_allclose(
        A.detach().cpu().numpy(), np.array([[1.5, 0.0], [0.0, 1.5]], dtype=float)
    )
    np.testing.assert_allclose(
        b.detach().cpu().numpy(), np.array([-1.0, -0.8], dtype=float)
    )
...
def test_torch_cg_solver_and_linear_system_fallbacks(monkeypatch: pytest.MonkeyPatch):
    ...
    np.testing.assert_allclose(
        sol.detach().cpu().numpy(), np.array([1.0, 1.0], dtype=float), atol=1e-10
    )
    ...
    np.testing.assert_allclose(
        delta.detach().cpu().numpy(), np.array([0.5, 0.25], dtype=float)
    )
    ...
    np.testing.assert_allclose(
        delta_cg.detach().cpu().numpy(), np.array([0.2, 0.3], dtype=float)
    )
    ...
    monkeypatch.setattr(
        ...
        lambda A, b: (_ for _ in ()).throw(
            RuntimeError("missing libtorch_cuda_linalg runtime")
        ),
    )
    monkeypatch.setattr(
        gn_runtime,
    ...
    np.testing.assert_allclose(
        delta_cuda.detach().cpu().numpy(), np.array([0.7, 0.8], dtype=float)
    )
...
def test_maybe_rollback_rolls_back_and_stops(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        gn_runtime,
        "function_set_array",
        lambda _fn, values: restored.setdefault(
...
def test_maybe_rollback_verbose_messages(
...
def test_torch_cg_nonfinite_rz_new_and_maybe_rollback_continue_paths(
    monkeypatch: pytest.MonkeyPatch,
):
    values = iter(
        [
