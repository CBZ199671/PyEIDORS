...
def test_constructor_success_forward_measurement_and_create_image(monkeypatch):
    ...
    np.testing.assert_allclose(rec._forward_measurement(np.array([1.0, 2.0, 3.0])), np.array([1.0, 2.0]))
...
def test_prepare_jacobian_and_hierarchy_use_enabled_cache_manager(monkeypatch):
    monkeypatch.setattr(sparse_module, "model_signature_from_forward_model", lambda _fwd: "model-sig")
    monkeypatch.setattr(sparse_module, "pattern_signature_from_forward_model", lambda _fwd: "pattern-sig")
    monkeypatch.setattr(sparse_module, "backend_signature_from_forward_model", lambda _fwd: "backend-sig")

    rec = sparse_module.SparseBayesianReconstructor.__new__(sparse_module.SparseBayesianReconstructor)
    rec.config = sparse_module.SparseBayesianConfig(cache_jacobian=True, coarse_levels=(4, 2), coarse_group_size=3)
    rec.n_elements = 4
    ...
    rec._eit_pde = SimpleNamespace(jacobian_wrt_parameter=lambda wrt: np.eye(3, 4, dtype=float))
...
def test_sparse_engine_noise_floor_and_forward_without_to_numpy():
    ...
    assert rec._estimate_noise_level(np.array([np.nan, np.nan], dtype=float)) == rec.config.noise_floor
