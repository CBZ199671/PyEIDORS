...
def apply_acceleration_profile_overrides(args, *, mesh_dim: int) -> None:
    ...
    profile = resolve_acceleration_profile(getattr(args, "acceleration_profile", DEFAULT_ACCELERATION_PROFILE))
    setattr(args, "acceleration_profile", profile)
    if int(mesh_dim) != 3 or not prefers_3d_gpu_pipeline(profile):
        return

    if hasattr(args, "forward_backend") and str(getattr(args, "forward_backend", DEFAULT_FORWARD_BACKEND)) == DEFAULT_FORWARD_BACKEND:
        setattr(args, "forward_backend", FORWARD_BACKEND_CUDA_STRUCTURED)
    if hasattr(args, "mesh_family") and str(getattr(args, "mesh_family", DEFAULT_MESH_FAMILY)) == DEFAULT_MESH_FAMILY:
        setattr(args, "mesh_family", MESH_FAMILY_HEX)
    if hasattr(args, "petsc_device") and str(getattr(args, "petsc_device", DEFAULT_PETSC_DEVICE)) == DEFAULT_PETSC_DEVICE:
    ...
    if prefers_fused_3d_gpu_pipeline(profile):
        if hasattr(args, "rom_mode") and str(getattr(args, "rom_mode", DEFAULT_ROM_MODE)) == DEFAULT_ROM_MODE:
            setattr(args, "rom_mode", "on")
        if hasattr(args, "inexact_mode") and str(getattr(args, "inexact_mode", DEFAULT_INEXACT_MODE)) == DEFAULT_INEXACT_MODE:
            setattr(args, "inexact_mode", "auto")
        if hasattr(args, "lowrank_mode") and str(getattr(args, "lowrank_mode", DEFAULT_LOWRANK_MODE)) == DEFAULT_LOWRANK_MODE:
...
def resolve_3d_mesh_contract(
    *,
    ...
    resolved_family = (
        ...
        else str(mesh_family or DEFAULT_MESH_FAMILY).strip().lower() or DEFAULT_MESH_FAMILY
    )
    resolved_geometry = (
        DEFAULT_3D_GEOMETRY_VERSION
        if wants_gpu and geometry_version is None
        else str(geometry_version or DEFAULT_3D_GEOMETRY_VERSION).strip().lower() or DEFAULT_3D_GEOMETRY_VERSION
    )
    resolved_revision = (
        DEFAULT_3D_GENERATOR_REVISION
        if wants_gpu and generator_revision is None
        else str(generator_revision or DEFAULT_3D_GENERATOR_REVISION).strip().lower() or DEFAULT_3D_GENERATOR_REVISION
    )
