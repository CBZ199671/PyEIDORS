...
TRANSLATIONS: dict[str, str] = {
    ...
    "sim.mesh.size_label": "Mesh density:",
    "sim.mesh.refinement_tooltip": "Controls how many target segments span the domain diameter D. Default is about D/18. Internally this is converted to generation scale h and integer refinement; measure actual cells from generated edge-length or diameter statistics.",
    "sim.mesh.density_mark.coarse": "Coarse",
    "sim.mesh.density_mark.medium": "Medium",
    "sim.mesh.density_mark.fine": "Fine",
    "sim.mesh.density_mark.very_fine": "Very fine",
    "sim.mesh.density_summary": "D/{density} · h≈{target:.3f} m · refinement≈{refinement} · estimated elements≈{cells}",
    "sim.mesh.density_advanced_toggle": "Advanced input",
    "sim.mesh.density_advanced_label": "Density value:",
    "sim.mesh.density_spin_suffix": " seg",
    "sim.mesh.density_advanced_tooltip": "Target segment count across domain diameter D; higher values produce finer meshes.",
    "sim.mesh.density_warning": "High density can greatly increase element count and solve time, especially in 3D.",
    ...
    "sim.mesh.complex_high_accuracy_toggle": "High accuracy for complex GPU",
    "sim.mesh.complex_high_accuracy_tooltip": "Affects only 3D GPU complex forward solves; enabled uses the strict-residual block-real AmgX route.",
    "sim.mesh.complex_high_accuracy_hint": "High accuracy can be much slower. When off, complex GPU uses native complex64 CUDA fgmres+gamg; real GPU still uses AmgX by default.",
    ...
    "sim.inverse.method.noser_sparse.tooltip": (
        "NOSER sparse route: solve the single-step 3D difference inverse problem "
        "with a linearized/matrix-free operator and no RM artifact."
    ),
    ...
    # ==================================================================
    "preflight.skipped": "Connection preflight skipped.",
    "preflight.serial.no_port.summary": "No serial port selected.",
    "preflight.serial.no_port.hint": "Click Scan first, or pick an auto-detected port from the dropdown.",
    "preflight.serial.win_absent.summary": "Windows did not detect serial port {name}.",
    "preflight.serial.win_absent.hint": "Make sure the device is still plugged in; re-plug USB and click Scan if needed. If you just closed the app, wait 1-2 s and retry.",
    "preflight.serial.win_present.summary": "Detected Windows serial port {name}.",
    "preflight.serial.win_present.hint": "{name} detected; the connection opens automatically through the Windows host serial bridge. If you just closed the app, a short retry runs automatically.",
    "preflight.serial.dev_missing.summary": "Serial device {port} not found.",
    "preflight.serial.dev_missing.hint": "Check the USB cable and device power; if you're running under WSL, prefer an auto-detected Windows COM port from the dropdown.",
    "preflight.serial.cannot_open.summary": "Serial port {name} cannot be opened right now.",
    "preflight.serial.ok.summary": "Serial port {name} confirmed openable.",
    "preflight.serial.ok.hint": "{name} detected; serial preflight passed, continuing to verify the device protocol.",
    "preflight.serial.fail.denied": "{port} access denied — it may be held by another program. Close the holding process and retry.",
    "preflight.serial.fail.missing": "{port} does not exist or isn't ready. Check the USB connection and Scan again.",
    "preflight.serial.fail.unconfigurable": "{port} cannot be configured right now. If you're on WSL, don't type /dev/ttyS* manually — prefer an auto-detected Windows COM port.",
    "preflight.serial.fail.generic": "{port} cannot be opened right now. Verify the port number, driver and baud-rate settings.",
    "preflight.relay.no_host.summary": "4G relay server address is empty.",
    "preflight.relay.no_host.hint": "Enter a reachable server host before connecting.",
    "preflight.relay.fail.summary": "Could not connect to the 4G relay server {host}:{port}.",
    "preflight.relay.ok.summary": "Connected to 4G relay server {host}:{port}.",
    "preflight.relay.ok.hint": "Server {host}:{port} is reachable; continuing to verify the device handshake and link capability.",
    "preflight.relay.fail.refused": "{host}:{port} refused the connection. Make sure the relay service is running and the host/port are correct.",
    "preflight.relay.fail.timeout": "Connection to {host}:{port} timed out. Check the network, server address and whether the target device is online.",
    "preflight.relay.fail.dns": "Could not resolve server address {host}. Check the host spelling or DNS configuration.",
    "preflight.relay.fail.generic": "{host}:{port} is unreachable. Check the network, service status and firewall settings.",
    "interop.env.runtime.wsl": "WSL bridged",
    "interop.env.runtime.linux": "Linux native",
    "interop.env.runtime.windows": "Windows native",
    "interop.env.runtime.unknown": "Unlabeled",
    "interop.env.name.full": "{matlab} / EIDORS ({runtime})",
    "interop.env.name.matlab_only": "{matlab} ({runtime}, startup pending)",
    "interop.env.name.startup_only": "EIDORS startup ({runtime}, MATLAB pending)",
    "interop.env.name.generic": "EIDORS environment ({runtime})",
    "interop.env.err.powershell_unavailable": "PowerShell is unavailable.",
    "interop.env.err.powershell_timeout": "PowerShell call timed out.",
    "interop.env.err.command_timeout": "Command execution timed out.",
    "interop.env.issue.scanned_windows": "Scanned common Windows-side MATLAB/EIDORS locations, including Program Files/MATLAB on every filesystem drive and directories like workspace/source/GitHub/Desktop/Downloads.",
    "interop.env.issue.scanned_linux": "Scanned common Linux/WSL-side MATLAB/EIDORS locations, including $MATLABROOT, $EIDORS_HOME, ~/workspace, ~/src, /opt, /usr/local.",
    "interop.env.issue.octave_only": "Detected Octave, but the v1 interop chain still uses MATLAB as the official runtime; Octave is kept as a future extension slot.",
    "interop.env.issue.toolbox_first": "Looked for EIDORS startup.m in the toolbox under the detected MATLAB install directory first.",
    "interop.env.issue.broadened": "MATLAB was found, but with no toolbox hit the search broadened automatically to common project directories to keep looking for startup.m.",
    "interop.env.issue.no_matlab": "No MATLAB install path was auto-detected; set it manually in Profiles & Paths.",
    "interop.env.issue.no_startup": "No EIDORS startup.m was auto-detected; click Browse to set it manually. After picking a user script, the system also tries to infer it from the script location.",
    "interop.env.issue.incomplete": "Some environment info was detected, but MATLAB or startup.m is still missing; complete it manually before capturing scripts.",
    "interop.env.issue.none": "No usable candidate environment detected.",
    "interop.env.test.matlab.no_command": "MATLAB executable not configured yet.",
    "interop.env.test.matlab.launch_failed": "Failed to launch MATLAB: {error}",
    "interop.env.test.matlab.nonzero": "MATLAB returned a non-zero exit code.",
    "interop.env.test.matlab.ok": "MATLAB launched.",
    "interop.env.test.startup.missing_paths": "Both the MATLAB path and EIDORS startup.m must be configured.",
    "interop.env.test.startup.failed": "EIDORS startup test failed: {error}",
    "interop.env.test.startup.exec_failed": "EIDORS startup execution failed.",
    "interop.env.test.startup.ok": "EIDORS startup ran.",
    "interop.svc.err.boundary_uninterpretable": "Boundary voltage data found, but it can't be interpreted directly with the current configuration: {error}",
    "interop.svc.err.exporter_mesh_only": "The exporter only supports triangular 2D meshes or tetrahedral 3D meshes.",
    "interop.svc.err.need_2d_coords": "At least 2D node coordinates are required to infer electrode nodes.",
    "interop.svc.err.no_boundary_nodes": "Could not infer boundary nodes from the geometry.",
    "interop.svc.err.no_matlab": "MATLAB executable path is not configured yet.",
    "interop.svc.err.no_startup": "EIDORS startup.m is not configured yet.",
    "interop.svc.note.capture_report": "Generated capture_report.json; view the raw capture result under Diagnostics.",
    "interop.svc.err.need_env_for_script": "Select a MATLAB/EIDORS environment before capturing an EIDORS script.",
    "interop.svc.err.unsupported_source": "Only importing a bridge directory, a legacy .mat file, or an EIDORS .m script is supported.",
    "interop.svc.note.no_geometry_export": "This export didn't include geometry.mat; the current source hasn't provided a reusable geometry payload yet.",
    "interop.svc.note.no_measurements_export": "This export didn't include boundary voltage data; the current source has no exportable measurement arrays.",
    "interop.svc.err.smoke_needs_two": "Smoke validation needs both homogeneous and target boundary voltages.",
    "interop.svc.smoke.compat_ok": "Data compatibility check passed: {count} boundary voltage points can be interpreted with the current layout.",
    "interop.svc.smoke.compat_3d_suffix": " This is a 3D configuration; the auto smoke test skips full inverse by default and only completes input compatibility validation.",
    "interop.svc.smoke.inverse_ok": "Inverse smoke test passed: {count} boundary voltage points ran through {method}, producing a reconstruction over {n_elements} elements.",
    "diag.amgx_unavailable_spd_gamg": "AmgX unavailable, using spd_gamg CUDA",
    "dlg.interop.status.bridge_label": "Bridge package",
}
